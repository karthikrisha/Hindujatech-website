    ...
    <Layout {...props}>
      <section className={styles.brochures}>
        <div className="container">
          ...
          <h1 className="title">Category: {props.data?.name}</h1>
...
export async function getStaticPaths() {
  const paths = await getPathData('/categories');
...
export async function getStaticProps({params}) {
  ...
  try {
    const props = await getCommonPageData(`/categories/find/${slug}`);
    console.log(`Building slug: /categories/find/${slug}`)
