                    ...
                    {careersList?.map((career) => (
                      <div key={career?.id} className={styles.box}>
                        <div>
                          ...
                          <a href={career?.attachment?.url} className="btn" target="_blank">View Job Details</a>
                        </div>
                      </div>
                    ))}
...
export async function getStaticProps(appContext) {
  try {
    let props = await getCommonPageData(`/lca-postings?_limit=-1`);
